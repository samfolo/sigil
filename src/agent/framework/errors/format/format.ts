...
import {AGENT_ERROR_CODES} from '@sigil/src/agent/framework/errors/codes';
import type {
	AgentError,
	ApiErrorContext,
...
} from '@sigil/src/agent/framework/errors/types';
...
export const formatAgentErrorsForDeveloper = (
	...
	if (grouped['error']) {
		const errorSection = [
			...
			...grouped['error'].map((err) => `- ${formatAgentError(err)}`),
		].join('\n');
	...
	if (grouped['warning']) {
		const warningSection = [
			...
			...grouped['warning'].map((err) => `- ${formatAgentError(err)}`),
		].join('\n');

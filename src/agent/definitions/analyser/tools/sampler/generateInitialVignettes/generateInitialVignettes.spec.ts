...
/**
 * Timeout for generateInitialVignettes operations
 */
const GENERATE_INITIAL_VIGNETTES_TIMEOUT = 15000; // 15 seconds
...
describe('generateInitialVignettes', () => {
	...
	it(
		'returns correct number of vignettes',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'returns all chunks when count exceeds available',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'vignettes have valid 384-dimensional embeddings',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'vignettes have valid positions',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'positions point to correct content in rawData',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'state tracks provided indices correctly',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'handles whitespace-only data with error',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'handles negative count with error',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'invokes progress callbacks with correct arguments',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
	...
	it(
		'works without callbacks (backwards compatible)',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
			const data = 'Sample text. '.repeat(SAMPLE_TEXT_REPEAT_MEDIUM);
			const result = await generateInitialVignettes(data, SAMPLE_VIGNETTE_COUNT_MEDIUM);
	...
	it('continues execution when chunking callback throws error',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
			const data = 'Sample text. '.repeat(SAMPLE_TEXT_REPEAT_MEDIUM);
			const onChunkingComplete = vi.fn(() => {
	...
	it(
		'continues execution when embedding progress callback throws error',
		{timeout: GENERATE_INITIAL_VIGNETTES_TIMEOUT},
		async () => {
			const data = 'Sample text. '.repeat(SAMPLE_TEXT_REPEAT_MEDIUM);
			const onEmbeddingProgress = vi.fn(() => {

...
describe('ParseYAMLStructureMetadataDetailsSchema', () => {
	const SIZE = {bytes: 10, characters: 10, lines: 1};
	const DEPTH = {value: 1, exact: true};
	const METADATA_STRING = {structure: 'string', size: SIZE};
	const METADATA_OBJECT = {structure: 'object', topLevelKeys: [{value: 'key', exact: true}], totalKeyCount: 1, depth: DEPTH, size: SIZE};
	...
	it.each([
		...
		const result = ParseYAMLStructureMetadataDetailsSchema.safeParse(input);
